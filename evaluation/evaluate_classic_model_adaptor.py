...
from adaptor.evaluators.question_answering import F1ScoreForQA, ExactMatch
...
if args.task == "QA":
    ...
    evaluator = ExactMatch() if args.metric == "EM" else F1ScoreForQA()
    predictor = FinetunedQAHeadPredictor(model, tokenizer)

    predictions, references = predictor.predict(id_dataset)
    id_results = evaluator.evaluate_str(references, predictions)
    report("ID", id_results)

    predictions, references = predictor.predict(id_dataset)
    ood_results = evaluator.evaluate_str(references, predictions)
